...
testPromise("empty", (_) => {
  ...
  Most.observe((_) => raise(Failure("this shouldn't happen")), empty) |> Js.Promise.then_(success);
});

testAsync("just", done_ => {
  open Expect;
...
testPromise("fromPromise", (_) => {
  let promise = Js.Promise.resolve("OK");
  let result = ref("");
  let success = Helpers.asyncExpectToEqual("OK");
  Most.fromPromise(promise) |> Most.observe(res => result := res) |> Js.Promise.then_((_) => success(result^));
});
...
testPromise("map", (_) => {
  ...
  Most.(from([|1, 2, 3|]) |> map(n => n * 2) |> observe(combineResult) |> Js.Promise.then_(success));
});

testPromise("constant", (_) => {
  let result = [||];
  let combine = Helpers.combineArray(result);
...
testPromise("scan", (_) => {
  ...
  Most.from([|1, 2, 3, 4|]) |> Most.scan((accum, n) => accum + n, 0) |> Most.observe(combine) |> Js.Promise.then_(success);
});

testPromise("flatMap", (_) => {
  let result = [||];
  let combine = Helpers.combineArray(result);
  let success = (_) => Helpers.asyncExpectToEqual([|0, 1, 0, 2, 0, 3, 0, 4|], result);
  Most.from([|1, 2, 3, 4|]) |> Most.flatMap(x => Most.from([|0, x|])) |> Most.observe(combine) |> Js.Promise.then_(success);
});
...
testPromise("filter", (_) =>
  ...
  |> Js.Promise.then_(result => Js.Promise.resolve(Expect.(expect(result) |> toEqual([6, 4, 2]))))
);
 ...
 * Slicing
 **/
testPromise("slice", (_) => Expect.(Most.(fromList([1, 2, 3, 4, 5, 6]) |> slice(2, 5) |> reduce((acc, n) => [n, ...acc], [])) |> Js.Promise.(then_(result => resolve(expect(result) |> toEqual([5, 4, 3]))))));

testPromise("take", (_) => Expect.(Most.(from([|1, 2, 3, 4, 5, 6|]) |> take(3) |> reduce((acc, n) => [n, ...acc], [])) |> Js.Promise.(then_(result => resolve(expect(result) |> toEqual([3, 2, 1]))))));

testPromise("skip", (_) => Expect.(Most.(from([|1, 2, 3, 4, 5, 6|]) |> skip(3) |> reduce((acc, n) => [n, ...acc], [])) |> Js.Promise.(then_(result => resolve(expect(result) |> toEqual([6, 5, 4]))))));
...
testPromise("merge", (_) => {
  ...
  merge(s1, s2) |> reduce((acc, n) => [n, ...acc], []) |> Js.Promise.then_(result => Js.Promise.resolve(Expect.expect(result) |> Expect.toEqual([String("c"), Int(3), String("b"), Int(2), String("a"), Int(1)])));
});

testPromise("combine", (_) => {
  open Most;
  let s1 = fromList([Int(1), Int(2), Int(3)]);
  let s2 = fromList([String("a"), String("b"), String("c")]);
  let toTuple = (a, b) => (a, b);
  ...
  |> reduce((acc, n) => [n, ...acc], [])
  |> Js.Promise.(then_(result => resolve(Expect.(expect(result) |> toEqual([(Int(3), String("c")), (Int(3), String("b")), (Int(2), String("b")), (Int(2), String("a")), (Int(1), String("a"))])))));
});
...
testPromise("zip", (_) => {
  ...
  let s2 = fromList([String("a"), String("b"), String("c")]);
  let toTuple = (a, b) => (a, b);
  zip(toTuple, s1, s2) |> reduce((acc, n) => [n, ...acc], []) |> Js.Promise.(then_(result => resolve(Expect.(expect(result) |> toEqual([(Int(3), String("c")), (Int(2), String("b")), (Int(1), String("a"))])))));
});

testPromise("sample1", (_) => {
  open Most;
  open Expect;
  let id = x => x;
  let sampler = periodic(100) |> take(10);
  let s = periodic(200) |> constant(2) |> startWith(1);
  sample1(id, sampler, s) |> reduce((acc, n) => [n, ...acc], []) |> Js.Promise.(then_(result => resolve(expect(result) |> toEqual([2, 2, 2, 2, 2, 2, 2, 2, 2, 1]))));
});

testPromise("sample2", (_) => {
  open Most;
  open Expect;
  let add = (x, y) => x + y;
  let sampler = periodic(100) |> take(10);
  ...
  sample2(add, sampler, s1, s2) |> reduce((acc, n) => [n, ...acc], []) |> Js.Promise.(then_(result => resolve(expect(result) |> toEqual([3, 3, 3, 3, 3, 3, 3, 3, 3, 3]))));
});
 ...
 * Subjects!
 **/
testPromise("basic Subject", (_) => {
  open Most;
  open Expect;
  let subj = Subject.make();
  let stream = Subject.asStream(subj);
  let promise = reduce((acc, n) => [n, ...acc], [], stream) |> Js.Promise.then_(result => Js.Promise.resolve(expect(result) |> toEqual([3, 2, 1])));
  Subject.next(1, subj);
  Subject.next(2, subj);
  Subject.next(3, subj);
